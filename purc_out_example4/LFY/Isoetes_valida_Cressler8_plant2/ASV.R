...
pdf("Isoetes_valida_Cressler8_plant2_read_lengths.pdf")
hist(lens, 100, xlim = c(min(lens)-1, max(lens)+1), xlab = "Length (bp)", main = "Isoetes_valida_Cressler8_plant2")
...
pdf("Isoetes_valida_Cressler8_plant2_error_profile.pdf")
plotErrors(err)
dev.off()
...
for (i in 1:dim(st.nochim)[2]){
  newName <- paste("Isoetes_valida_Cressler8_plant2_ASV", i, sep = "")
  outputNames <- c(outputNames, newName)
}
uniquesToFasta(st.nochim, "Isoetes_valida_Cressler8_plant2_ASVs.fasta", ids=outputNames)
print(format(Sys.time()))
quit()
