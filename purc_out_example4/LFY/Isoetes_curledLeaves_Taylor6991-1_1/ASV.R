...
pdf("Isoetes_curledLeaves_Taylor6991-1_1_read_lengths.pdf")
hist(lens, 100, xlim = c(min(lens)-1, max(lens)+1), xlab = "Length (bp)", main = "Isoetes_curledLeaves_Taylor6991-1_1")
...
pdf("Isoetes_curledLeaves_Taylor6991-1_1_error_profile.pdf")
plotErrors(err)
dev.off()
...
for (i in 1:dim(st.nochim)[2]){
  newName <- paste("Isoetes_curledLeaves_Taylor6991-1_1_ASV", i, sep = "")
  outputNames <- c(outputNames, newName)
}
uniquesToFasta(st.nochim, "Isoetes_curledLeaves_Taylor6991-1_1_ASVs.fasta", ids=outputNames)
print(format(Sys.time()))
quit()
