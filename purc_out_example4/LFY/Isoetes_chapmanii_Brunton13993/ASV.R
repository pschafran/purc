...
pdf("Isoetes_chapmanii_Brunton13993_read_lengths.pdf")
hist(lens, 100, xlim = c(min(lens)-1, max(lens)+1), xlab = "Length (bp)", main = "Isoetes_chapmanii_Brunton13993")
...
pdf("Isoetes_chapmanii_Brunton13993_error_profile.pdf")
plotErrors(err)
dev.off()
...
for (i in 1:dim(st.nochim)[2]){
  newName <- paste("Isoetes_chapmanii_Brunton13993_ASV", i, sep = "")
  outputNames <- c(outputNames, newName)
}
uniquesToFasta(st.nochim, "Isoetes_chapmanii_Brunton13993_ASVs.fasta", ids=outputNames)
print(format(Sys.time()))
quit()
