...
pdf("Isoetes_microvela_JBNC202-EO5-B_read_lengths.pdf")
hist(lens, 100, xlim = c(min(lens)-1, max(lens)+1), xlab = "Length (bp)", main = "Isoetes_microvela_JBNC202-EO5-B")
...
pdf("Isoetes_microvela_JBNC202-EO5-B_error_profile.pdf")
plotErrors(err)
dev.off()
...
for (i in 1:dim(st.nochim)[2]){
  newName <- paste("Isoetes_microvela_JBNC202-EO5-B_ASV", i, sep = "")
  outputNames <- c(outputNames, newName)
}
uniquesToFasta(st.nochim, "Isoetes_microvela_JBNC202-EO5-B_ASVs.fasta", ids=outputNames)
print(format(Sys.time()))
quit()
