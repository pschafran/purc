...
def ReverseComplement(seq):
	...
	basecomplement = {'A': 'T', 'T': 'A', 'C': 'G', 'G': 'C', 'N': 'N', 'R': 'Y', 'Y':'R', 'M': 'K', 'K': 'M', 'S': 'S', 'W': 'W', 'H': 'D', 'D': 'H', 'B': 'V', 'V': 'B', 'I': 'N'} # Make a dictionary for complement
	letters = list(seq) 
...
def CheckChimericLoci_byBC(inputfile_raw_sequences, outputfile_goodSeqs, outputfile_chimeras, barcode_seq_filename, SeqDict):
	#usearch_cLine = "%s -search_local %s -db %s -strand both -target_cov 0.75 -evalue 0.01 -maxhits 8 -userout %s -userfields query+target+alnlen+id+bits+qlo+qhi -threads %d" % (Usearch, inputfile_raw_sequences, barcode_seq_filename, Output_folder+'/search_barcode_out_maxhit8.txt', num_threads)
	#os.popen(usearch_cLine)	

	search_result = open(Output_folder+'/search_barcode_out_maxhit8.txt', 'rU')
	good_seq = open(outputfile_goodSeqs, 'w')
	chimera_seq = open(outputfile_chimeras, 'w')

	seq_barcode_dict = {}
	for line in search_result:
		line = line.strip('\n')
		seq_name = line.split('\t')[0]
		barcode_name = line.split('\t')[1]
		barcode_start = int(line.split('\t')[5])
		barcode_end = int(line.split('\t')[6])
		try:
			seq_barcode_dict[seq_name].append([barcode_name, barcode_start, barcode_end])
		except:
			seq_barcode_dict[seq_name] = [[barcode_name, barcode_start, barcode_end]]

	chimera_seq_list = []
	chimera_seq_split_list = []
	for seq in seq_barcode_dict:
		seq_barcode_dict[seq].sort(key=lambda x: x[1]) # sort by barcode_start
		barcode_coordinate_list = seq_barcode_dict[seq]
		if len(barcode_coordinate_list) == 4 and barcode_coordinate_list[0][2] < 100: # make sure the first barcode is at the beginning of the seq
			if barcode_coordinate_list[0][0][2] != barcode_coordinate_list[1][0][2]: # F or R of the first two barcodes
				if barcode_coordinate_list[1][1] - barcode_coordinate_list[0][2] <= 2000: # the distance between the barcodes can't be longer than 1500bp
					new_seq = str(SeqDict[seq].seq)[:barcode_coordinate_list[1][2]]
					good_seq.write('>'+seq+'_split1'+'\n'+new_seq+'\n')
					chimera_seq_split_list.append(seq)	
			if barcode_coordinate_list[2][0][2] != barcode_coordinate_list[3][0][2]: # F or R of the first two barcodes
				if barcode_coordinate_list[3][1] - barcode_coordinate_list[2][2] <= 2000: # the distance between the barcodes can't be longer than 1500bp
					new_seq = str(SeqDict[seq].seq)[barcode_coordinate_list[2][1]:]
					good_seq.write('>'+seq+'_split2'+'\n'+new_seq+'\n')
					chimera_seq_split_list.append(seq)	
		
		elif len(barcode_coordinate_list) == 3 and barcode_coordinate_list[0][2] < 100:
			if barcode_coordinate_list[0][0][2] != barcode_coordinate_list[1][0][2]: # F or R of the first two barcodes
				if barcode_coordinate_list[1][1] - barcode_coordinate_list[0][2] <= 2000: # the distance between the barcodes can't be longer than 1500bp
					new_seq = str(SeqDict[seq].seq)[:barcode_coordinate_list[1][2]]
					good_seq.write('>'+seq+'_split1'+'\n'+new_seq+'\n')
		...
		if len(barcode_coordinate_list) > 2:
			chimera_seq.write('>'+seq+'\n'+str(SeqDict[seq].seq)+'\n')
			chimera_seq_list.append(seq)
		# else:
		# 	good_seq.write('>' + str(seq) + '\n' + str(SeqDict[seq].seq) + '\n')
			
	good_seq_list = list(set(list(SeqDict.keys())) - set(chimera_seq_list))	
	for seq in good_seq_list:
		good_seq.write('>' + str(seq) + '\n' + str(SeqDict[seq].seq) + '\n')

	return len(set(chimera_seq_split_list))
...
def DeBarcoder_dual(inputfile_raw_sequences, SeqDict, SearchOnlyEnds=True, search_range=50):
	"""Search the raw sequences with barcodes, identifies the barcode, adds the barcode ID to the 
	...
	# Get sequences from two ends and concat them in "tempFR_barcode"
	if SearchOnlyEnds:
		FR_ends = open('tempFR_barcode', 'w')
		for each_rec in sorted(SeqDict):
			seq_to_search_F = str(SeqDict[each_rec].seq)[:search_range]
			seq_to_search_R = str(SeqDict[each_rec].seq)[-search_range:]
			FR_ends.write('>' + str(each_rec) + '\n' + seq_to_search_F + seq_to_search_R + '\n')
		FR_ends.close()
		
		# Use usearch's search_local function to find barcodes; it works better than ublast and search_oligodb	
		usearch_cLine = "%s -search_local %s -db %s -strand both -target_cov 0.5 -evalue 1 -maxhits 2 -userout %s -userfields query+target+alnlen+id+bits+qlo+qhi -threads %d" % (Usearch, 'tempFR_barcode', barcode_seq_filename, Output_folder+'/search_barcode_out.txt', num_threads)
		os.popen(usearch_cLine)	
	else:
		usearch_cLine = "%s -search_local %s -db %s -strand both -target_cov 0.5 -evalue 1 -maxhits 2 -userout %s -userfields query+target+alnlen+id+bits+qlo+qhi -threads %d" % (Usearch, inputfile_raw_sequences, barcode_seq_filename, Output_folder+'/search_barcode_out.txt', num_threads)
		os.popen(usearch_cLine)	

	bc_search_hits = open(Output_folder + '/search_barcode_out.txt', 'rU') # Read the blast result	
	...
	if not SearchOnlyEnds:
		#bc_toomany = open(Output_folder + '/' + Output_prefix + '_1_trashBin_tooMany_bc.fa', 'w') # For saving those more than one barcode
		bc_mid = open(Output_folder + '/' + Output_prefix + '_1_trashBin_midbc.fa', 'w') # For saving barcodes in the middle of seq
	...
	for each_rec in bc_search_hits:
		each_rec = each_rec.strip('\n')
		seq_name = each_rec.split('\t')[0]
		barcode_name = each_rec.split('\t')[1] # E.g. BC01, BC24...
		barcode_start_posi = int(each_rec.split('\t')[5])
	...
	for each_seq in set(seq_withbc_list):
		# When more than three barcodes are present in seq; not relevant when forcing -maxhits 2
		...
		# When only one barcode is present in seq
			...
			# Check the orientation of the sequence; if the F barcode is in the 3' end or R in 5' end, reverse complement the seq		
			if int(barcode_info_dict[each_seq][0][1]) <= search_range and barcode_info_dict[each_seq][0][0][2] == 'F': 
				new_seq_trimmed = str(SeqDict[each_seq].seq[barcode_info_dict[each_seq][0][2]:]) 
			elif int(barcode_info_dict[each_seq][0][1]) <= search_range and barcode_info_dict[each_seq][0][0][2] == 'R': 
				new_seq_trimmed = ReverseComplement(str(SeqDict[each_seq].seq[barcode_info_dict[each_seq][0][2]:])) 
			elif int(barcode_info_dict[each_seq][0][1]) >= search_range and barcode_info_dict[each_seq][0][0][2] == 'R': 
				if SearchOnlyEnds:
					new_seq_trimmed = str(SeqDict[each_seq].seq[:len(str(SeqDict[each_seq].seq))-search_range*2+barcode_info_dict[each_seq][0][1]-1])
				else:
					new_seq_trimmed = str(SeqDict[each_seq].seq[:barcode_info_dict[each_seq][0][1]-1])
			elif int(barcode_info_dict[each_seq][0][1]) >= search_range and barcode_info_dict[each_seq][0][0][2] == 'F': 
				if SearchOnlyEnds:			
					new_seq_trimmed = ReverseComplement(str(SeqDict[each_seq].seq[:len(str(SeqDict[each_seq].seq))-search_range*2+barcode_info_dict[each_seq][0][1]-1]))
				else:
					new_seq_trimmed = ReverseComplement(str(SeqDict[each_seq].seq[:barcode_info_dict[each_seq][0][1]-1]))
			else: # Those barcodes that are at the middle of the sequences; not relevant when only two ends are searched
				if not SearchOnlyEnds:
					new_seq_name = new_seq_name + "ERRmidBC"
					bc_mid.write('>' + new_seq_name + '\n' + str(SeqDict[each_seq].seq) + '\n')
		...
		# When both barcodes are present in seq
			...
			if barcode_info_dict[each_seq][0][0][2] == barcode_info_dict[each_seq][1][0][2]:
				...
				continue 
			if SearchOnlyEnds:
				# Check if the two barcodes are on the same side; don't want these 
				if int(barcode_info_dict[each_seq][0][1]) <= search_range and int(barcode_info_dict[each_seq][1][1]) <= search_range: 
					new_seq_name = str(barcode_info_dict[each_seq][0][0]) + '^' + str(barcode_info_dict[each_seq][1][0]) + 'same_side' + '|' + str(each_seq) # Add the barcode ID to the sequence name: BC01|sequence_name
					bc_invalid.write('>' + new_seq_name + '\n' + str(SeqDict[each_seq].seq) + '\n')
					continue 
				# Check if the two barcodes are on the same side; don't want these 
				if int(barcode_info_dict[each_seq][0][1]) >= search_range and int(barcode_info_dict[each_seq][1][1]) >= search_range: 
			...
			if SearchOnlyEnds:
				if int(barcode_info_dict[each_seq][0][1]) <= search_range: 
					new_seq_trimmed = str(SeqDict[each_seq].seq[barcode_info_dict[each_seq][0][2]:len(str(SeqDict[each_seq].seq))-search_range*2+barcode_info_dict[each_seq][1][1]-1]) 
					bc_trimmed.write('>' + new_seq_name + '\n' + new_seq_trimmed + '\n')
				# BCR - R primer - Seq - F primer - BCF
				elif int(barcode_info_dict[each_seq][0][1]) >= search_range: # When the seq is reverse complemented
					new_seq_trimmed = str(SeqDict[each_seq].seq[barcode_info_dict[each_seq][1][2]:len(str(SeqDict[each_seq].seq))-search_range*2+barcode_info_dict[each_seq][0][1]-1]) 
	...
	if SearchOnlyEnds:
		os.remove('tempFR_barcode')

def DeBarcodePrimer_dual(inputfile_raw_sequences, barcode_primer_fasta, SeqDict, Output_folder, Output_prefix):
	"""Use the whole barcoded primer sequences to demultiplex, not very sensitive - don't use"""
	
	usearch_cLine = "usearch -search_local %s -db %s -strand both -evalue 1 -maxhits 2 -userout %s -userfields query+target+qlo+qhi+tlo+thi+alnlen+evalue+qstrand -threads %d" % (inputfile_raw_sequences, barcode_primer_fasta, Output_folder+'/barcode_primer_hits.txt', num_threads)
	os.popen(usearch_cLine)	
	
	bc_hits = open(Output_folder + '/barcode_primer_hits.txt', 'rU') # Read the blast result
	bc_trimmed = open(Output_folder + '/' + Output_prefix + '_1_bc_primer_trimmed.fa', 'w') # For writing the de-barcoded sequences
	bc_leftover = open(Output_folder + '/' + Output_prefix + '_1_trashBin_no_bc.fa', 'w') # For saving those without barcodes
	bc_onlyF = open(Output_folder + '/' + Output_prefix + '_1_trashBin_onlyF_bc.fa', 'w')
	bc_onlyR = open(Output_folder + '/' + Output_prefix + '_1_trashBin_onlyR_bc.fa', 'w')
	bc_toomany = open(Output_folder + '/' + Output_prefix + '_1_trashBin_tooMany_bc.fa', 'w') # For saving those more than one barcode
	...
	barcode_info_dict = {} # {seq_name1: [(BCF01, 0, 12), (BCR02, 459, 511)], seq_name2: [(BC08, 0, 12)]}; barcode_info_dict[seq_name] = [(barcodeF_name, barcode_start_posi, barcode_end_posi), (barcodeR_name, barcode_start_posi, barcode_end_posi)]
							# each seq has a list of tuples, each tuple contains the (barcode name, start, end)
	# Go through the blast output file, and complete the barcode_info_dict, and seq_withbc_list
	for each_rec in bc_hits:
		each_rec = each_rec.strip('\n')
		seq_name = each_rec.split('\t')[0]
		barcode_name = each_rec.split('\t')[1].split('|')[0] # E.g. BC01, BC24...
		locus_name = each_rec.split('\t')[1].split('|')[1]
		barcode_start_posi = int(each_rec.split('\t')[2])
		barcode_end_posi = int(each_rec.split('\t')[3])		
		seq_withbc_list.append(seq_name)
		# {seq_name1: [(BCF01, rbcLX, 0, 12), (BCR02, rbcLX, 459, 511)], seq_name2: [(BC08, AMF, 0, 12)]}
		try:
			barcode_info_dict[seq_name].append((barcode_name, locus_name, barcode_start_posi, barcode_end_posi))
		except:
			barcode_info_dict[seq_name] = [(barcode_name, locus_name, barcode_start_posi, barcode_end_posi)]
	...
	for each_seq in set(seq_withbc_list):
		...
		# When only one barcode is present in seq
			...
			new_seq_name = str(barcode_info_dict[each_seq][0][1]) + '|' + str(barcode_info_dict[each_seq][0][0]) + '|' + str(each_seq) # Add the barcode ID to the sequence name: BC01|sequence_name
			# Check the orientation of the sequence; if the barcode is in the 3' end, reverse complement the seq		
			if barcode_info_dict[each_seq][0][2] < 5: # The start position _should_ be 1, but this allows for some slop
				new_seq_trimmed = str(SeqDict[each_seq].seq[barcode_info_dict[each_seq][0][3]:]) # "barcode_end_posi" to the end of the sequence
			elif barcode_info_dict[each_seq][0][2] > len(str(SeqDict[each_seq].seq))-30: # Those barcodes that are at the end of the sequences, so need to be reversecomplemented
				new_seq_trimmed = ReverseComplement(str(SeqDict[each_seq].seq[:barcode_info_dict[each_seq][0][2]-1])) # the beginning of the sequence to "barcode_start_posi" - 1
			else: # Those barcodes that are at the middle of the sequences
				new_seq_trimmed = str(SeqDict[each_seq].seq[barcode_info_dict[each_seq][0][3]:])
				new_seq_name = new_seq_name + "ERRmidBC"
			# Check where barcode is located, on F or R primer
			if barcode_info_dict[each_seq][0][0][2] == 'F':
				bc_onlyF.write('>' + new_seq_name + '\n' + new_seq_trimmed + '\n')
		...
		# When both barcodes are present in seq
			...
			# Check if barcodes are in the same category (F, R); don't want these 
			if barcode_info_dict[each_seq][0][0][2] == barcode_info_dict[each_seq][1][0][2]:
				new_seq_name = str(barcode_info_dict[each_seq][0][1]) + '|' + str(barcode_info_dict[each_seq][0][0]) + '^' + str(barcode_info_dict[each_seq][1][0]) + '|' + str(each_seq) # Add the barcode ID to the sequence name: BC01|sequence_name
				bc_invalid.write('>' + new_seq_name + '\n' + str(SeqDict[each_seq].seq) + '\n')
				continue 
			# Check if both primers come from the same locus
			if barcode_info_dict[each_seq][0][1] != barcode_info_dict[each_seq][1][1]:
				new_seq_name = str(barcode_info_dict[each_seq][0][1]) + str(barcode_info_dict[each_seq][0][2]) + '|' + str(barcode_info_dict[each_seq][0][0]) + '^' + str(barcode_info_dict[each_seq][1][0]) + '|' + str(each_seq) # Add the barcode ID to the sequence name: BC01|sequence_name
				bc_invalid.write('>*' + new_seq_name + '\n' + str(SeqDict[each_seq].seq) + '\n')
				continue
			...
			new_seq_name = str(barcode_info_dict[each_seq][0][1]) + '|' + str(barcode_info_dict[each_seq][0][0]) + '^' + str(barcode_info_dict[each_seq][1][0]) + '|' + str(each_seq) # Add the barcode ID to the sequence name: BCF01|BCR02|sequence_name

			# Trim the barcodes 
			# BCF - F primer - Seq - R primer - BCR
			if int(barcode_info_dict[each_seq][0][2]) <= 5: 
				new_seq_trimmed = str(SeqDict[each_seq].seq[barcode_info_dict[each_seq][0][3]:barcode_info_dict[each_seq][1][2]-1]) 
				bc_trimmed.write('>' + new_seq_name + '\n' + new_seq_trimmed + '\n')
			# BCR - R primer - Seq - F primer - BCF
			elif int(barcode_info_dict[each_seq][0][2]) >= len(str(SeqDict[each_seq].seq))-30: # When the seq is reverse complemented
				new_seq_trimmed = str(SeqDict[each_seq].seq[barcode_info_dict[each_seq][1][3]:barcode_info_dict[each_seq][0][2]-1]) 
				new_seq_trimmed = ReverseComplement(new_seq_trimmed)		
				bc_trimmed.write('>' + new_seq_name + '\n' + new_seq_trimmed + '\n')
			else:
				new_seq_name = new_seq_name + "ERRmidBC"
...
def LocusAssigner(inputfile, primer_fasta, DualBC_flag=True, search_range=25):
	""" Search primer, assign each seq to locus and sample, remove the primer seq """
	...
	for each_file, each_locus in zip(mapping_file_list, locus_list): 
		dictOfMapDicts[each_locus.upper()] = makeMapDict(each_file, each_locus, False, DualBC_flag) # Note the Multiplex_perBC and DualBC flags (as True/False)
	
	SeqDict = SeqIO.index(inputfile, 'fasta') 

	# Get sequences from two ends and concat them in "tempFR_barcode"
	FR_ends = open('tempFR_primer', 'w')
	for each_rec in sorted(SeqDict):
		seq_to_search_F = str(SeqDict[each_rec].seq)[:search_range]
		seq_to_search_R = str(SeqDict[each_rec].seq)[-search_range:]
		FR_ends.write('>' + str(each_rec) + '\n' + seq_to_search_F + seq_to_search_R + '\n')
	...
	usearch_cLine = "%s -search_local %s -db %s -strand both -evalue 1e-1 -maxhits 2 -userout %s -userfields query+target+qlo+qhi+tlo+thi+alnlen+evalue+qstrand -threads %d" % (Usearch, 'tempFR_primer', primer_fasta, Output_folder+'/search_primer_out.txt', num_threads)
	os.popen(usearch_cLine)	
	
	primer_hits = open(Output_folder + '/search_primer_out.txt', 'rU') # Read the blast result
	locus_assigned = open(Output_folder + '/' + Output_prefix + '_2_annotated.fa', 'w') # For writing the de-barcoded sequences
	locus_not_assigned = open(Output_folder + '/' + Output_prefix + '_2_locus_assigned_failed.fa', 'w') # For writing the de-barcoded sequences
	unclassifiable = open(Output_folder + '/' + Output_prefix + '_2_bc_conflict.fa', 'w') # For writing the de-barcoded sequences

	seq_locus_assigned_list = []
	primer_info_dict = {} 
	LocusTaxonCountDict = {}

	# Go through the usearch output file, and complete the primer_info_dict, and seq_locus_assigned_list
	for each_rec in primer_hits:
		each_rec = each_rec.strip('\n')
		seq_name = each_rec.split('\t')[0]
		primer_name = each_rec.split('\t')[1].split('|')[0] # e.g. seq name in primer fasta has to be like: cw|rbcLX; here cw is grabbed
		locus_name = each_rec.split('\t')[1].split('|')[1].upper() # e.g. seq name in primer fasta has to be like: cw|rbcLX; here RBCLX is grabbed
		primer_start_posi = int(each_rec.split('\t')[2])
		primer_end_posi = int(each_rec.split('\t')[3])		
		seq_locus_assigned_list.append(seq_name)
		#primer_info_dict[seq_name] = locus_name
		try:
			primer_info_dict[seq_name].append((locus_name, primer_start_posi, primer_end_posi))
		except:
			primer_info_dict[seq_name] = [(locus_name, primer_start_posi, primer_end_posi)]
	
	# Annotate and write sequences
	for each_seq in set(seq_locus_assigned_list):
		locus_name = primer_info_dict[each_seq][0][0]
		try: # Use try/except to avoid the error when the key is not present in MapDict				
			key = str(each_seq).split('|')[0] # e.g. BCF1^BCR2
			taxon_name = dictOfMapDicts[locus_name][key] 
			new_seq_name = taxon_name + '|' + locus_name + '|' + str(each_seq) # Add the taxon name and locus name to the sequence name: Apunctatus|RBCLX|BCF1^BCR4|sequence_name
			try:
				LocusTaxonCountDict[taxon_name, locus_name] += 1 #as {('C_mem_6732', 'PGI'): 2, ('C_mem_6732', 'IBR'): 4} for example
			except:
				LocusTaxonCountDict[taxon_name, locus_name] = 1
		except: # This barcode combination isn't found in the dict
			new_seq_name = locus_name + '|' + str(each_seq)
			unclassifiable.write('>' + new_seq_name + '\n' + str(SeqDict[seq_name].seq) + '\n')
			continue

		if int(primer_info_dict[each_seq][0][1]) <= search_range: 
			new_seq_trimmed = str(SeqDict[each_seq].seq)[primer_info_dict[each_seq][0][2]:]
		if int(primer_info_dict[each_seq][0][1]) >= search_range: 
			new_seq_trimmed = str(SeqDict[each_seq].seq)[:len(str(SeqDict[each_seq].seq))-search_range*2+primer_info_dict[each_seq][0][1]-1]
		try: # if second primer is found
			if int(primer_info_dict[each_seq][1][1]) <= search_range: 
				new_seq_trimmed = new_seq_trimmed[primer_info_dict[each_seq][1][2]:]
		except:
			pass
		try:
			if int(primer_info_dict[each_seq][1][1]) >= search_range: 
				new_seq_trimmed = new_seq_trimmed[:len(new_seq_trimmed)-search_range*2+primer_info_dict[each_seq][1][1]-1]
		except:
			pass

		locus_assigned.write('>' + new_seq_name + '\n' + new_seq_trimmed + '\n')

	# Save those that no primer can be found
	seq_locus_not_assigned_list = list(set(list(SeqDict.keys())) - set(seq_locus_assigned_list))	
	for seq_withoutbc in seq_locus_not_assigned_list:
		locus_not_assigned.write('>' + str(seq_withoutbc) + '\n' + str(SeqDict[seq_withoutbc].seq) + '\n')

	os.remove('tempFR_primer')
...
if len(sys.argv) < 2:
	...
	for line in configuration:
		...
		if setting_line != -1:
			...
			elif setting_name == 'in_Primer_seq_file':	
				primer_seq_filename = setting_argument
				if not os.path.isfile(primer_seq_filename):
					sys.exit("Error: could not find " + primer_seq_filename)
...
if mode == 0: # QC mode
	...
	split_count = CheckChimericLoci_byBC(raw_sequences, Output_folder+'/'+Output_prefix+'_0_nonchimeras.fa', Output_folder+'/'+Output_prefix+'_0_chimeras.fa', barcode_seq_filename, SeqDict)
	...
	raw_sequences = Output_folder + '/' + Output_prefix+'_0_nonchimeras.fa'
	SeqDict = SeqIO.index(raw_sequences, 'fasta')
	sys.stderr.write('\t' + str(count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_0_chimeras.fa')) + ' inter-locus chimeric sequences found\n')
	log.write('\t' + str(count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_0_chimeras.fa')) + ' inter-locus chimeric sequences found\n')
	sys.stderr.write('\t' + str(split_count) + ' chimeric sequences successfully dechimerized\n')
	log.write('\t' + str(split_count) + ' chimeric sequences successfully dechimerized\n')
...
if Dual_barcode:
	...
	DeBarcoder_dual(raw_sequences, SeqDict, SearchOnlyEnds=Search_ends_only_flag)
	#DeBarcodePrimer_dual(raw_sequences, barcode_seq_filename, SeqDict, Output_folder, Output_prefix)

else:
	sys.stderr.write('Removing barcodes...\n')
	if not Search_ends_only_flag:
...
if Dual_barcode:
	count_seq_w_onlyF_bc = count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_1_trashBin_onlyF_bc.fa')
	count_seq_w_onlyR_bc = count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_1_trashBin_onlyR_bc.fa')
	count_seq_w_invalid_bc = count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_1_trashBin_invalid_bc.fa')
	if not Search_ends_only_flag:
		count_seq_w_mid_bc = count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_1_trashBin_midbc.fa')

sys.stderr.write('\t' + str(count_seq_w_bc) + ' sequences have correct barcode\n')
...
if Dual_barcode:
	sys.stderr.write('\t' + str(count_seq_w_onlyF_bc) + ' sequences have only F barcode\n')
	sys.stderr.write('\t' + str(count_seq_w_onlyR_bc) + ' sequences have only R barcode\n')
	sys.stderr.write('\t' + str(count_seq_w_invalid_bc) + ' sequences have invalid F-F or R-R barcode\n')
	if not Search_ends_only_flag:
		sys.stderr.write('\t' + str(count_seq_w_mid_bc) + ' sequences have barcode in the middle of sequence\n')
...
## Assign locus and remove primters ##
log.write('#Primer Removal#\n')
sys.stderr.write('Removing primers...\n')
LocusTaxonCountDict_unclustd = LocusAssigner(Output_folder + '/' + Output_prefix + '_1_bc_trimmed.fa', primer_seq_filename, DualBC_flag = Dual_barcode)

## Remove primers ##
# log.write('#Primer Removal#\n')
# sys.stderr.write('Removing primers...\n')
# primer_trimmed_file = Output_prefix + '_3_pr_trimmed.fa'
# doCutAdapt(Fprims = Forward_primer, Rprims = Reverse_primer, InFile = Output_folder + '/' + Output_prefix + '_2_annotated.fa', OutFile = Output_folder + '/' + primer_trimmed_file)
# count_seq_pr_trimmed = count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_3_pr_trimmed.fa')
...
count_seq_annotated = count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_2_annotated.fa')
count_seq_bc_conflict = count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_2_bc_conflict.fa')
count_seq_unclassifiable = count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_2_locus_assigned_failed.fa')
...
count_output = open(Output_prefix + '_6_counts.xls', 'w')
...
if mode == 0:
	count_output.write('Concatemers (multi-locus seqs):\t' + str(count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_0_chimeras.fa')) + '\n')
	log.write('Concatemers (multi-locus seqs):\t' + str(count_seq_from_fasta(Output_folder + '/' + Output_prefix + '_0_chimeras.fa')) + '\n')

count_output.write('Sequences with correct barcodes:\t' + str(count_seq_w_bc) + '\n')
...
if Dual_barcode:
	count_output.write('Sequences with only F barcode:\t' + str(count_seq_w_onlyF_bc) + '\n')
	count_output.write('Sequences with only R barcode:\t' + str(count_seq_w_onlyR_bc) + '\n')
	count_output.write('Sequences with invalid F-F or R-R barcode:\t' + str(count_seq_w_invalid_bc) + '\n')
	count_output.write('Sequences with invalid F-R or R-F barcode:\t' + str(count_seq_bc_conflict) + '\n')
	if not Search_ends_only_flag:
		count_output.write('Sequences with barcode in the middle:\t' + str(count_seq_w_mid_bc) + '\n')
count_output.write('Sequences annotated:\t' + str(count_seq_annotated) + '\n')
count_output.write('Sequences that cannot be assigned to locus:\t' + str(count_seq_unclassifiable) + '\n')

log.write('Sequences with correct barcodes:\t' + str(count_seq_w_bc) + '\n')
...
if Dual_barcode:
	log.write('Sequences with only F barcode:\t' + str(count_seq_w_onlyF_bc) + '\n')
	log.write('Sequences with only R barcode:\t' + str(count_seq_w_onlyR_bc) + '\n')
	log.write('Sequences with invalid F-F or R-R barcode:\t' + str(count_seq_w_invalid_bc) + '\n')
	log.write('Sequences with invalid F-R or R-F barcode:\t' + str(count_seq_bc_conflict) + '\n')
	if not Search_ends_only_flag:
		log.write('Sequences with barcode in the middle:\t' + str(count_seq_w_mid_bc) + '\n')
log.write('Sequences annotated:\t' + str(count_seq_annotated) + '\n')
log.write('Sequences that cannot be assigned to locus:\t' + str(count_seq_unclassifiable) + '\n')
